...
# TODO: remove amount restriction
for entry in data_raw_filtered[:2]:
    words_list = re.sub("[^a-zA-Z]", " ", entry['body'])
    data_clean[entry["id"]] = {"body": [w for w in words_list.lower().split()],
                               "topics": entry["topics"],
                               "id": entry["id"]}

print data_clean["7001"]

data_words_only = []

for article in data_clean:
    data_words_only.append(" ".join(data_clean[article]["body"]))  # glue all words together into a list of strings
